            ...
            for (Pokemon_stats * PokemonStat : pokemon_stats){
                if (PokemonStat->pokemon_id == this->id){
                    if (PokemonStat->stat_id == 1){
                        this->hp = PokemonStat->base_stat + rand() % 16;
                    }
                    else if (PokemonStat->stat_id == 2){
                        this->attack = PokemonStat->base_stat + rand() % 16;
                    }
                    else if (PokemonStat->stat_id == 3){
                        this->defense = PokemonStat->base_stat + rand() % 16;
                    }
                    else if (PokemonStat->stat_id == 4){
                        this->speed = PokemonStat->base_stat + rand() % 16;
                        this->baseSpeed = PokemonStat->base_stat;
                    }
                    else if (PokemonStat->stat_id == 5){
                        this->s_attack = PokemonStat->base_stat + rand() % 16;
                    }
                    else if (PokemonStat->stat_id == 6){
                        this->s_defense = PokemonStat->base_stat + rand() % 16;
                    }
                }
            }
        }
...
public:
    ...
    Map(int numTrainers){
        ...
        for (int i = 0; i < numCharacters; i++) {
            //characters[i] = static_cast<Character*>(malloc(sizeof(Character)));
...
int checkNumPokemon(Character* c){
    int i, numPoke;
    for (i = 0; i < 6; i++){
...
int changeMap(Map*** mapArray, Map ** m, int direction, int numPlayers, int* worldX, int* worldY){
    int newX, newY, oldPCx, oldPCy, newPCx, newPCy, i, pot, rev, bal, numPokemon;
    std::vector<Pokemon*> pokemon;
...
int enterPokeMart(Character * c){
    ...
    mvprintw(0, 0, "Entered PokeMart! All items have been restored!");
    mvprintw(0, 0, "Press '<' to exit");
    refresh();
    int ui = 'a';
    while (ui != '<'){
        ui = getch();
...
int enterPokeCenter(Character* c){
    ...
    mvprintw(0, 0, "Entered PokeCenter! All of your pokemon have been");
    mvprintw(1, 0, "restored to max hp! Press '<' to exit");
    refresh();
    int ui = 'a';
    while (ui != '<'){
        ui = getch();
...
void printTrainerBattle(Character* pc, Character* npc, int op, int mp){
    ...
    mvprintw(4,0, "press 'b' to use an bag item or 'p' to switch pokemon, or 's' to view the other trainer's pokemon");
...
int showNpcPokemon(Character * npc){
    ...
    for (i = 0; i < npc->num_pokemon; i ++){
        ...
        snprintf(buff, sizeof(buff), "POKEMON %d: \n    %s (%s), level: %d, move 1: %s, move 2: %s \n    stats: hp: %d, attack: %d, defense: %d, speed: %d\n    s_attack: %d, s_defense: %d, gender: %s", i + 1, npc->pokemonArray[i]->name.c_str(), npc->pokemonArray[i]->isShiny.c_str(),npc->pokemonArray[i]->level, npc->pokemonArray[i]->moveSet[0]->name.c_str(), npc->pokemonArray[i]->moveSet[1]->name.c_str(), npc->pokemonArray[i]->hp, npc->pokemonArray[i]->attack, npc->pokemonArray[i]->defense, npc->pokemonArray[i]->speed, npc->pokemonArray[i]->s_attack, npc->pokemonArray[i]->s_defense, npc->pokemonArray[i]->gender.c_str());
...
int enterBattle(Character * pc, Character * npc){
    int i, chosenPokemon, otp, ui, moveIdx, turnP, opm, idx, crit, damage, aod, random, stab;
    chosenPokemon = -1;
    //npc->defeated = 0;
...
void printMap(Map* m, Character ** players, int numPlayers, int worldX, int worldY){
    ...
    //mvprintw(23, 0, "User's pokemon gender: %s", m->characters[0]->pokemonArray[0]->gender.c_str());
...
int main(int argc, char *argv[]){
    ...
    mvprintw(1, 0, "1: (%s), 2: (%s), 3: (%s)", newPokemon1->name.c_str(), newPokemon2->name.c_str(), newPokemon3->name.c_str());
    ...
    while (chosen == 1){
        ...
        if (response == '1'){
            ...
            mvprintw(2, 0, "Please type '1', '2', or '3' to choose a pokemon:");
        }
    }
